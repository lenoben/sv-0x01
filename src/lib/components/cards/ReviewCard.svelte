...
>
	...
	<div class="flex items-center flex-wrap gap-2 text-xs sm:text-sm mb-4">
		{#each reviewItem.features as keywork}
			<div
				class="p-2 py-1 rounded-md border border-solid border-indigo-400 bg-indigo-50 text-indigo-400 dark:border-indigo-500 dark:bg-slate-900 dark:text-slate-50"
			>
				<p>{keywork}</p>
			</div>
		{/each}
